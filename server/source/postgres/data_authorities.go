...
var infos = []postgres.DataAuthorityId{
	{888, 888},
	{888, 8881},
	{888, 9528},
	{9528, 8881},
...
func (d *dataAuthorities) Init() error {
	return global.GVA_DB.Table("data_authority_ids").Transaction(func(tx *gorm.DB) error {
		if tx.Where("sys_data_authority_id IN (888, 9528) ").Find(&[]DataAuthority{}).RowsAffected == 5 {
			color.Danger.Println("\n[Postgres] --> sys_data_authority_ids 表初始数据已存在!")
			return nil
		}
		...
		color.Info.Println("\n[Postgres] --> sys_data_authority_ids 表初始数据成功!")
		return nil
	})
}
