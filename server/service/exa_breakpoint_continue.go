...
func FindOrCreateFile(fileMd5 string, fileName string, chunkTotal int) (err error, file postgres.ExaFile) {
	var cfile postgres.ExaFile
...
func CreateFileChunk(id uint, fileChunkPath string, fileChunkNumber int) error {
	var chunk postgres.ExaFileChunk
...
func DeleteFileChunk(fileMd5 string, fileName string, filePath string) error {
	var chunks []postgres.ExaFileChunk
	var file postgres.ExaFile
