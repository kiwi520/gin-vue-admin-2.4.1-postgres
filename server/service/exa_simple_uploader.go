...
func SaveChunk(uploader postgres.ExaSimpleUploader) (err error) {
...
func CheckFileMd5(md5 string) (err error, uploads []postgres.ExaSimpleUploader, isDone bool) {
	err = global.GVA_DB.Find(&uploads, "identifier = ? AND is_done = ?", md5, false).Error
	isDone = errors.Is(global.GVA_DB.First(&postgres.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, true).Error, gorm.ErrRecordNotFound)
...
func MergeFileMd5(md5 string, fileName string) (err error) {
	...
	if !errors.Is(global.GVA_DB.First(&postgres.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, true).Error, gorm.ErrRecordNotFound) {
		return nil
	}
	...
	err = global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		//删除切片信息
		if err = tx.Delete(&postgres.ExaSimpleUploader{}, "identifier = ? AND is_done = ?", md5, false).Error; err != nil {
			fmt.Println(err)
			return err
		}
		data := postgres.ExaSimpleUploader{
			Identifier: md5,
			IsDone:     true,
